...
Page({
  ...
  onShow() {
    ...
    app.globalData.historyListListener = (payload) => {
      ...
      const list = rawList.map((item) => {
        const isSpec = item.result === 'spectator';
        const factionClass = isSpec ? 'spec' : (EVIL_ROLES.has(item.role) ? 'evil' : 'good');
        const cardClass = item.result === 'win' ? 'win' : isSpec ? 'spec' : 'lose';
        return {
          ...item,
          roleImage: isSpec ? '' : (ROLE_IMAGE_MAP[item.role] || ''),
          spectatorAvatar: isSpec ? userAvatar : '',
      ...
      const pageDisplay = String(page).padStart(2, '0');
      this.setData({
        list,
        page,
        pageDisplay,
  ...
  formatPeerVoteSummary(summary = {}) {
    const parts = [];
    if (summary.c_le) parts.push(`MVP ${summary.c_le}`);
  ...
  onOpenDetail(e) {
    ...
    wx.navigateTo({ url: `/subpkg/history-detail/index/index?gameId=${gameId}` });
  }
});
